...
public class UserOrderService {
    ...
    public List<OrderAdminResponse> getMyOrders(String email) {
        return orderRepository.findAllByUserEmailOrderByIdDesc(email).stream()
                .map(this::toResponse)
                .toList();
    }

    public OrderAdminResponse getMyOrderDetail(Long id, String email) {
        Order order = orderRepository.findDetailedByIdAndUserEmail(id, email)
